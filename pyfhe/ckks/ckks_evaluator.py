...
from pyfhe.ckks.ckks_bootstrapping_context import CKKSBootstrappingContext
from pyfhe.util.ciphertext import Ciphertext
from pyfhe.util.crt import CRTContext
...
class CKKSEvaluator:
    ...
    def multiply_matrix_naive(self, ciph, matrix, rot_keys, encoder):
        ...
        """
        diag = pyfhe.util.matrix_operations.diagonal(matrix, 0)
        diag = encoder.encode(diag, self.scaling_factor)
        ciph_prod = self.multiply_plain(ciph, diag)

        for j in range(1, len(matrix)):
            diag = pyfhe.util.matrix_operations.diagonal(matrix, j)
            diag = encoder.encode(diag, self.scaling_factor)
    ...
    def multiply_matrix(self, ciph, matrix, rot_keys, encoder):
        ...
        for j in range(matrix_len_factor2):
            ...
            for i in range(matrix_len_factor1):
                diagonal = pyfhe.util.matrix_operations.diagonal(matrix, shift + i)
                diagonal = pyfhe.util.matrix_operations.rotate(diagonal, -shift)
